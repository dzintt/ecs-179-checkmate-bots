...
@export var cross_width: int = 2  # Width of the cross spawn area (always 2)

# Colors for different areas
var chess_tile_color_light: Color = Color("White")
var chess_tile_color_dark: Color = Color("Black")
var cross_color_light: Color = Color("Dark Green")
var cross_color_dark: Color = Color("Dark Olive Green")
...
func _create_cross_area():
	...
	for x in range(chess_board_size * 2 + cross_width):
		for y in range(cross_width):
			var tile = _create_tile(Vector2(x * tile_size, (chess_board_size + y) * tile_size), (x + y) % 2 == 0, true)
	...
	for x in range(cross_width):
		for y in range(chess_board_size * 2 + cross_width):
			...
			var tile = _create_tile(
				Vector2((chess_board_size + x) * tile_size, y * tile_size),
				(x + y) % 2 == 0,
				true
			)
...
func _create_tile(pos: Vector2, is_light: bool, is_spawn: bool) -> ColorRect:
	...
	if is_spawn:
		tile.color = cross_color_light if is_light else cross_color_dark
	else:
		tile.color = chess_tile_color_light if is_light else chess_tile_color_dark
